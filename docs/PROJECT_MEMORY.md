...
- 当前已完成的是前 3 项的基础实现和 M0-M3 基础工作；AI 对话尚未实现。
...
| M3 项目级 iServer 数据中心 | 核心实现完成，生命周期未闭环 | 已有项目选择、服务登记、列表、元数据、预览、删除和用户/项目隔离；真实文件上传、发布/取消发布仍需补齐。 |
...
- 当前没有真正的上传文件、`publish`、`unpublish` API；下一轮不要把“登记服务”描述成“已完成数据发布”。
...
4. M3 数据中心的服务登记不是完整 iServer 服务目录同步，也不等同于本地文件上传和发布。
...
```text
python -m pytest tests -q       -> 24 passed, 44 warnings
node --test tests/js/*.test.cjs -> 7 passed
python -m compileall -q server scripts tests -> passed
```
...
2. 若要先完成团队四项功能，优先补齐 M3 的真实上传/发布闭环，再在有 iServer/SuperMap3D 的机器上验证 M1。
3. 之后进入 M4：导入合法、可发放的洛南案例数据，消除计分路径中的随机/模拟值，并生成可追溯报告。
